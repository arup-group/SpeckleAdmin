  ...
  <v-container grid-list-xl>
    ...
    <v-layout row wrap>
      ...
      <v-flex xs12>
        ...
        <v-btn icon @click.native='pageNumber+=1' :disabled='pageNumber >= Math.ceil(filteredStreams.length/sliceSize) - 1'>
          <v-icon>chevron_right</v-icon>
        </v-btn>
        <v-btn icon small @click.native='pageNumber=Math.ceil(filteredStreams.length/sliceSize) - 1' :disabled='pageNumber >= Math.ceil(filteredStreams.length/sliceSize) - 1'>
          <v-icon>last_page</v-icon>
        </v-btn>
        <span class='caption' xxxstyle="position: relative;top:8px;">{{pageNumber + 1}} / {{Math.ceil(filteredStreams.length/sliceSize).toFixed(0)}}</span>
      </v-flex>
