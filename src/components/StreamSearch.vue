<template>
  <v-layout align-center row wrap @mouseleave='mouseLeave'>
    <v-flex xs12 class='pb-0'>
      <v-text-field ref="searchField" box flat clearable prepend-inner-icon="search" label='search for streams' @input="updateSearch" v-model='searchfilter' spellcheck="false" :disabled='globalDisabled' :loading='searchInProgress' append-icon="refresh" @click:append="$store.dispatch( 'getStreams', 'parent:type=10&omit=objects,layers,viewerLayers&isComputedResult=false&sort=updatedAt&limit=500' )"></v-text-field>
    </v-flex>
